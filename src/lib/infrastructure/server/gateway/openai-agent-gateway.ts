...
import { generateOpenAIAssistantIDFromRCExternalID, generateOpenAiAssistantInstructions, generateOpenAIAssistantName, generateRCExternalIDFromOpenAIAssistantID, uint8ArrayToBase64 } from "../config/openai/openai-utils";
...
export default class OpenAIAgentGateway implements AgentGatewayOutputPort<TOpenAIMessageContext> {
  ...
  async createAgent(researchContextTitle: string, researchContextDescription: string, vectorStoreID: string, additionalFiles?: RemoteFile[], agentSystemInstructions?: string): Promise<TCreateAgentDTO> {
    ...
    this.logger.debug({ additionalFiles }, "DEBUG: Agent creation -- additional files");
    try {
